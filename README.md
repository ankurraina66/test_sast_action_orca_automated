...
| scan_info_hyperlinks | If `service_url` is configured in GitHub Actions secrets, `scan_info_hyperlinks` must be set to `false`. | true |
| github_token | To view the PR scan summary directly in the PR comment section when triggered, set the value to \`{{github.token}}\`. | github.token|
